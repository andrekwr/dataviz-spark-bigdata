...
if __name__ == "__main__":
    ...
    rdd_freq_oreo = (
        rdd_oreo.flatMap(conta_palavras)
        .reduceByKey(lambda x, y: x + y)
        .map(lambda x: (x[0], math.log10(1 + x[1])))
        .cache()
    )
    rdd_freq_negresco = (
        rdd_negresco.flatMap(conta_palavras)
        .reduceByKey(lambda x, y: x + y)
        .map(lambda x: (x[0], math.log10(1 + x[1])))
        .cache()
    )
    ...
    rdd_oreo = (
        rdd_freq_oreo.subtractByKey(rdd_freq_negresco)
        .join(rdd_idf)
        .map(lambda x: (x[0], x[1][0] * x[1][1]))
        .takeOrdered(100, key=lambda x: -x[1])
    )
    ...
    rdd_negresco = (
        rdd_freq_negresco.subtractByKey(rdd_freq_oreo)
        .join(rdd_idf)
        .map(lambda x: (x[0], x[1][0] * x[1][1]))
        .takeOrdered(100, key=lambda x: -x[1])
    )
    ...
    ).generate_from_frequencies(frequencies=dict_freq)
    wordcloud_all.to_file("wordcloud_all.png")

    wordcloud_oreo = WordCloud(
        max_font_size=50, max_words=100, background_color="white"
    ).generate_from_frequencies(frequencies=dict_freq_o)
    wordcloud_oreo.to_file("wordcloud_oreo.png")

    wordcloud_negresco = WordCloud(
        max_font_size=50, max_words=100, background_color="white"
    ).generate_from_frequencies(frequencies=dict_freq_n)
